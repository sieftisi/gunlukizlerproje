<div class="ortadiv">
    ...
    $listele = $list->query("SELECT * FROM post_yonetimi WHERE post_id=$x");
    ...
    while ($row = $listele->fetch()) {
        ...
        echo "<div class='post'>";
        if ($postpp) {
            echo "<div class='kullanicipost'><a href='profil1.php?c=$k_id'><img src='$postpp' class='userpicpost'></a></div>"; // Eğer kullanıcı fotoğrafı varsa onu kullan
        } else {
        ...
        echo "<h2 class='postbaslik'><font color='white'>" . $row['baslik'] . "</font></h2>";
        echo "<div class='postfoto'>";
        if (!empty($a)) {
            echo "<a href='$a' target='_blank'><img src='$a' alt='Fotoğraf' class='onizleme'></a>";
        }
        echo "</div>";
        echo "<div class='postyazi'>&nbsp;&nbsp;&nbsp;&nbsp;" . $row['tammetin'] . "</div>";
        echo "</div></td></tr>";
